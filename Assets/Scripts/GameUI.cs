...
public class GameUI : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        GameManager.OnLocalResultsTextChanged += OnResultsTextChanged;
    }
    ...
    private void OnConnectedPlayersChanged(int players)
    {
        ...
        {
            _lobbyStatusText.text = $"Подключено игроков: {players}/2";
        }
    }
    ...
    private void OnMatchTimerChanged(float time)
    {
        if (_matchTimerText != null)
        {
            ...
            _matchTimerText.text = $"Таймер: {seconds} сек";
        }
    }
    ...
    private void OnCountdownTimerChanged(float time)
    {
        ...
        {
            int seconds = Mathf.CeilToInt(time);
            _countdownText.text = $"Матч через: {seconds}";
        }
    }

    private void OnResultsTextChanged(string results)
    {
